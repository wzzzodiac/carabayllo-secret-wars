...
import { createRenderer } from './renderer.js?v=phase5a-terrain-2';
import { createUI } from './ui.js?v=phase5a-terrain-2';
...
const humanError = code => ({
  ...
  invalid_terrain: 'That terrain is invalid.',
  ...
  terrain_too_steep: 'That ledge is too steep to drive onto. Use a jump.',
  no_jumps_remaining: 'No jumps remaining this turn.',
  player_missing: 'Your vehicle is no longer active.'
...
function makeDisplayRoom(room) {
  ...
  return {
    ...
    arena: room.arena ? {
      ...room.arena,
      craters: (room.arena.craters ?? []).map(crater => ({ ...crater })),
      previewSpawns: [...(room.arena.previewSpawns ?? [])]
    } : null,
...
function syncTerrainOptions(room) {
  const entries = room?.terrainPresets ?? [];
  const signature = entries.map(entry => `${entry.id}:${entry.name}`).join('|');
  if (terrainSelect.dataset.signature !== signature) {
    ...
    terrainSelect.dataset.signature = signature;
  }
}

function updateTerrainControl(room) {
  syncTerrainOptions(room);
  ...
  const selectedName = room?.terrainPresets?.find(entry => entry.id === room.terrainPreset)?.name
    ?? terrainSelect.selectedOptions[0]?.textContent
    ?? '—';
...
function renderRoom(room) {
  ...
  const me = room?.players?.find(player => player.id === playerId);
  if (room?.match?.activePlayerId !== playerId || room?.match?.projectile || me?.alive === false) {
    stopHeldMove();
    clearPendingAim();
  }
...
function request(event, payload = {}) {
  return new Promise(resolve => activeSocket.timeout(8000).emit(event, payload, (err, result) => resolve(
    err ? { ok: false, error: 'request_timeout' } : result || { ok: false, error: 'empty_response' }
  )));
}
...
terrainSelect.addEventListener('change', () => {
  const selectedName = terrainSelect.selectedOptions[0]?.textContent ?? 'Terrain';
  mutate('set_terrain', { terrain: terrainSelect.value }, `${selectedName} selected. Preview updated; READY states reset.`);
});
...
async function sendMoveStep() {
  ...
  try {
    ...
    if (!result.ok) {
      if (['movement_limit', 'terrain_too_steep', 'not_your_turn', 'shot_in_flight', 'player_missing'].includes(result.error)) stopHeldMove();
...
console.info('Orbital Artillery Phase 5A layered terrain preview client ready.');
