...
import { createRenderer } from './renderer.js?v=phase5a-terrain-1';
...
terrainGroup.innerHTML = '<span class="control-label" id="terrainControlLabel">TERRAIN // HOST</span><select id="terrainSelect" aria-label="Terrain preset"><option value="rolling">ROLLING EXPANSE</option><option value="terraces">TERRACE LINE</option><option value="twinpeaks">TWIN PEAKS</option><option value="basin">IMPACT BASIN</option><option value="brokenridge">BROKEN RIDGE</option><option value="islands">DRIFT ISLANDS</option><option value="canyon">CANYON RUN</option></select></div>';
...
function makeDisplayRoom(room) {
  ...
  return {
    ...
    arena: room.arena ? { ...room.arena, craters: (room.arena.craters ?? []).map(crater => ({ ...crater })) } : null,
    players: room.players.map(player => {
      const next = { ...player, spawn: player.spawn ? { ...player.spawn } : null, motion: player.motion ? { ...player.motion } : null };
      ...
      if (next.spawn && previous && !next.motion && next.alive !== false) {
        ...
        if (moved) next.motion = { type: 'jump', startedAt: now, endsAt: now + MOVE_VISUAL_MS, fromX: previous.x, fromY: previous.y, toX: next.spawn.x, toY: next.spawn.y, apex: 0 };
      }
...
function updateTerrainControl(room) {
  const me = room?.players?.find(player => player.id === playerId);
  terrainGroup.hidden = !room || room.status !== 'lobby';
  terrainSelect.disabled = !me?.isHost || room?.status !== 'lobby';
  if (room?.terrainPreset) terrainSelect.value = room.terrainPreset;
  const selectedName = room?.terrainPresets?.find(entry => entry.id === room.terrainPreset)?.name ?? terrainSelect.selectedOptions[0]?.textContent ?? '—';
  terrainControlLabel.textContent = me?.isHost ? `TERRAIN // HOST // ${selectedName}` : `TERRAIN // ${selectedName}`;
}

function renderRoom(room) {
  currentRoom = room;
  if (room?.status === 'started' && ['INPUT', 'TEXTAREA', 'SELECT'].includes(document.activeElement?.tagName)) document.activeElement.blur();
  const me = room?.players?.find(player => player.id === playerId);
  if (room?.match?.activePlayerId !== playerId || room?.match?.projectile || me?.alive === false) { stopHeldMove(); clearPendingAim(); }
  updateTerrainControl(room);
...
async function ensureConnection() {
  ui.setBusy(true); ui.setServerStatus('CONNECTING');
  try {
    activeSocket = await socketBoundary.connect(); ui.setServerStatus('CONNECTED'); activeSocket.off('room_state'); activeSocket.on('room_state', renderRoom);
    if (!disconnectHandlerBound) { activeSocket.on('disconnect', () => { disconnectHandlerBound = false; stopHeldMove(); clearPendingAim(); ui.setServerStatus('OFFLINE'); ui.setMessage('Connection closed. Create or join again.'); }); disconnectHandlerBound = true; }
    return activeSocket;
...
async function joinRoom() { const value = name(); const code = ui.roomCode.value.trim().toUpperCase(); ui.roomCode.value = code; if (!value) return ui.setMessage('Enter a player name first.'); if (code.length !== 4) return ui.setMessage('Enter the 4-character room code.'); await ensureConnection(); const result = await request('join_room', { name: value, code }); if (!result.ok) return ui.setMessage(humanError(result.error)); playerId = result.playerId; renderRoom(result.room); ui.setMessage(`Joined room ${result.room.code}.`); }
...
terrainSelect.addEventListener('change', () => mutate('set_terrain', { terrain: terrainSelect.value }, `${terrainSelect.selectedOptions[0]?.textContent ?? 'Terrain'} selected. READY states reset.`));
...
async function sendMoveStep() {
  ...
  try {
    ...
    if (!result.ok) { if (['movement_limit', 'not_your_turn', 'shot_in_flight', 'player_missing'].includes(result.error)) stopHeldMove(); if (result.error !== 'movement_limit') ui.setMessage(humanError(result.error)); return; }
...
function queueAim(angleDelta = 0, powerDelta = 0) {
  if (!isMyActionTurn()) return; pendingAngleDelta += angleDelta; pendingPowerDelta += powerDelta; if (aimPumpPromise) return;
  aimPumpPromise = (async () => {
    while (isMyActionTurn() && (pendingAngleDelta || pendingPowerDelta)) {
      const angleStep = pendingAngleDelta, powerStep = pendingPowerDelta; pendingAngleDelta = 0; pendingPowerDelta = 0; const payload = {};
      ...
      const result = await request('set_aim', payload); if (!result.ok) { clearPendingAim(); ui.setMessage(humanError(result.error)); break; } renderRoom(result.room);
    }
  })().finally(() => { aimPumpPromise = null; });
}

async function fireShot() { stopHeldMove(); if (aimPumpPromise) await aimPumpPromise; if (!isMyActionTurn()) return; await mutate('fire_projectile', {}, 'Shot fired.'); }

window.addEventListener('keydown', event => {
  if (!isMyActionTurn()) return; const code = event.code; if (['KeyA', 'KeyD', 'KeyW', 'KeyS', 'KeyQ', 'KeyE', 'KeyF', 'Space'].includes(code)) event.preventDefault();
  if (code === 'KeyA' || code === 'KeyD') { if (!event.repeat) startHeldMove(code === 'KeyA' ? -1 : 1); return; }
  if (code === 'Space' && !event.repeat) { stopHeldMove(); const me = currentRoom.players.find(player => player.id === playerId); mutate('jump_player', { direction: me?.spawn?.facing || 1 }); return; }
  if (code === 'KeyW' || code === 'KeyS') { stopHeldMove(); queueAim(code === 'KeyW' ? 3 : -3, 0); return; }
  if (code === 'KeyQ' || code === 'KeyE') { stopHeldMove(); queueAim(0, code === 'KeyE' ? 5 : -5); return; }
...
window.addEventListener('pagehide', () => { stopHeldMove(); clearPendingAim(); combatControls.destroy(); windGusts.destroy(); socketBoundary.disconnect(); });

console.info('Orbital Artillery Phase 5A destructible terrain client ready.');
