...
import { createRenderer } from './renderer.js?v=phase6b-turnflow-1';
...
async function fireShot(){stopHeldMove();if(aimPumpPromise)await aimPumpPromise;if(!isMyActionTurn())return;const me=currentRoom.players.find(player=>player.id===playerId),slot=me?.selectedItemSlot??1,item=slot>1?me?.inventory?.[slot-2]:null;const label=item?.label??'Basic shot';await mutate('fire_projectile',{},item?.type==='shield'?'Shield activated — turn continues.':`${label} fired.`);}
...
console.info('Orbital Artillery Phase 6B AFK voting ready.');
