...
import { createRenderer } from './renderer.js?v=phase6a-items-1';
...
async function joinRoom(){const value=name(),code=ui.roomCode.value.trim().toUpperCase();ui.roomCode.value=code;if(!value)return ui.setMessage('Enter the 4-character room code.');if(code.length!==4)return ui.setMessage('Enter the 4-character room code.');await ensureConnection();const result=await request('join_room',{name:value,code});if(!result.ok)return ui.setMessage(humanError(result.error));playerId=result.playerId;renderRoom(result.room);ui.setMessage(`Joined room ${result.room.code}.`);}
...
async function fireShot(){stopHeldMove();if(aimPumpPromise)await aimPumpPromise;if(!isMyActionTurn())return;const me=currentRoom.players.find(player=>player.id===playerId),slot=me?.selectedItemSlot??0,item=slot>0?me?.inventory?.[slot-1]:null;await mutate('fire_projectile',{},item?.type==='heavy'?'Heavy Bomb fired.':'Shot fired.');}
function selectItem(slot){if(!currentRoom||!activeSocket||currentRoom.status!=='started')return;const me=currentRoom.players.find(player=>player.id===playerId);if(!me||me.alive===false)return;mutate('select_item',{slot},slot===0?'Basic shot selected.':`Inventory slot ${slot} selected.`);}

window.addEventListener('keydown',event=>{const code=event.code;if(['Digit0','Digit1','Digit2'].includes(code)&&!event.repeat){event.preventDefault();selectItem(Number(code.slice(-1)));return;}if(!isMyActionTurn())return;if(['KeyA','KeyD','KeyW','KeyS','KeyQ','KeyE','KeyF','Space'].includes(code))event.preventDefault();if(code==='KeyA'||code==='KeyD'){if(!event.repeat)startHeldMove(code==='KeyA'?-1:1);return;}if(code==='Space'&&!event.repeat){stopHeldMove();const me=currentRoom.players.find(player=>player.id===playerId);mutate('jump_player',{direction:me?.spawn?.facing||1});return;}if(code==='KeyW'||code==='KeyS'){stopHeldMove();queueAim(code==='KeyW'?3:-3,0);return;}if(code==='KeyQ'||code==='KeyE'){stopHeldMove();queueAim(0,code==='KeyE'?5:-5);return;}if(code==='KeyF'&&!event.repeat)fireShot();});
...
console.info('Orbital Artillery Phase 6A pickups, inventory and Heavy Bomb client ready.');
