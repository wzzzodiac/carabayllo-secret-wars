...
import { createRenderer } from './renderer.js?v=phase6b-items-1';
import { createUI } from './ui.js?v=phase5b-combat-1';
import { initWindGusts } from './wind-gusts.js?v=phase3-wind-1';
import { initCombatControls } from './combat-controls.js?v=phase6b-items-1';
...
function isMyActionTurn(){const me=currentRoom?.players.find(player=>player.id===playerId);return currentRoom?.status==='started'&&currentRoom.match?.activePlayerId===playerId&&!currentRoom.match?.projectile&&!currentRoom.match?.specialAction&&me?.alive!==false;}
...
async function fireShot(){stopHeldMove();if(aimPumpPromise)await aimPumpPromise;if(!isMyActionTurn())return;const me=currentRoom.players.find(player=>player.id===playerId),slot=me?.selectedItemSlot??1,item=slot>1?me?.inventory?.[slot-2]:null;const label=item?.label??'Basic shot';await mutate('fire_projectile',{},item?.type==='shield'?'Shield activated.':`${label} fired.`);}
...
console.info('Orbital Artillery Phase 6B Triple Shot, Cluster Bomb and Shield client ready.');
