export function createMatchStatsPanel({ onRematch } = {}) {
  ...
  function table(stats){return `<div style="overflow:auto"><table style="width:100%;border-collapse:collapse;font:700 12px ui-monospace,monospace"><thead><tr style="text-align:left;opacity:.7"><th>PLAYER</th><th>DMG</th><th>K</th><th>A</th><th>TAKEN</th><th>PICKUPS</th><th>BIGGEST</th></tr></thead><tbody>${stats.map((s,i)=>`<tr style="border-top:1px solid rgba(140,180,255,.14)"><td style="padding:8px 8px 8px 0">${i+1}. ${esc(s.name)}</td><td>${number(s.damageDealt)}</td><td>${number(s.kills)}</td><td>${number(s.assists)}</td><td>${number(s.damageReceived)}</td><td>${number(s.pickups)}</td><td>${number(s.biggestHit)}</td></tr>`).join('')}</tbody></table></div>`;}
  ...
  function update(room,playerId){
    ...
    const top=`<div style="display:flex;justify-content:space-between;gap:16px;align-items:flex-start;flex-wrap:wrap"><div><div class="section-label">PHASE 7A.1 // MATCH STATS</div><strong>${room.status==='finished'?resultText(room):'LIVE SCOREBOARD'}</strong>${summary?`<div style="margin-top:5px;opacity:.7;font:700 11px ui-monospace,monospace">${number(summary.turns)} turns // ${(number(summary.durationMs)/60000).toFixed(1)} min // top damage ${esc(summary.topDamage?.name??'—')} ${number(summary.topDamage?.value)}</div>`:''}</div>${room.status==='finished'&&me?.isHost?'<div style="display:flex;gap:8px;flex-wrap:wrap"><button type="button" data-rematch="same">REMATCH // SAME MAP</button><button type="button" data-rematch="random">REMATCH // RANDOM MAP</button></div>':''}</div>`;
    root.innerHTML=top+table(stats)+feed(room.eventFeed);
  }
  root.addEventListener('click',event=>{const button=event.target.closest?.('[data-rematch]');if(!button)return;button.disabled=true;onRematch?.(button.dataset.rematch==='random');});
