export function createMatchStatsPanel({ onRematch } = {}) {
  ...
  function update(room,playerId){
    ...
    const top=`<div style="display:flex;justify-content:space-between;gap:16px;align-items:flex-start;flex-wrap:wrap"><div><div class="section-label">VERSION 0.9.7 PRE-RELEASE // MATCH STATS</div><strong>${room.status==='finished'?resultText(room):'LIVE SCOREBOARD'}</strong>${summary?`<div style="margin-top:5px;opacity:.7;font:700 11px ui-monospace,monospace">${details}</div>`:''}</div>${room.status==='finished'&&me?.isHost?'<div style="display:flex;gap:8px;flex-wrap:wrap"><button type="button" data-rematch="same">REMATCH // SAME MAP</button><button type="button" data-rematch="random">REMATCH // RANDOM MAP</button></div>':''}</div>`;
